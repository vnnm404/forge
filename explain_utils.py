...
def initialise_explainer(model, explanation_algorithm_name, explanation_epochs=200, explanation_lr=0.01, task='binary_classification', node_mask_type='attributes', edge_mask_type='object'):
    return Explainer(
        model=model,
...
def explanation_accuracy(ground_truth_explanation: GraphXAIExplanation, predicted_explanation: PyGExplanation):
    '''
...
def visualise_explanation(pred_explanation: PyGExplanation, gt_explanation: Optional[GraphXAIExplanation] = None, type='edge', save_img=True):
    '''
