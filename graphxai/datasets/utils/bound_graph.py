...
def build_bound_graph(
    ...
    **kwargs,
    ...
    if base_graph == "ba":
        if "n_ba" in kwargs:
            subgraph_generator = partial(
                nx.barabasi_albert_graph, n=kwargs["n_ba"], m=1
            )
        else:
    ...
    for i in range(num_subgraphs):
        ...
        shape_node_per_subgraph.append(
            pivot
        )  # This node represents the shape in the graph
        ...
        for j in range(i + 1, len(subgraphs)):
            ...
            for k in range(inter_sg_connections):
                ...
                while len(possible_edges) > 0:
                    ...
                    possible_edges.remove(
                        rand_edge
                    )  # Remove b/c we're searching this edge possibility
                    ...
                    for t in list(original_shapes[i].nodes) + list(
                        original_shapes[j].nodes
                    ):
                        khop_union = khop_union.union(
                            set(
                                khop_subgraph_nx(node_idx=t, num_hops=num_hops, G=tempG)
                            )
                        )
