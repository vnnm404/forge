...
class EnclosingSubgraph:
    ...
    def draw(self, show=False):
        G = gxai_utils.to_networkx_conv(
            Data(edge_index=self.edge_index), to_undirected=True
        )
        nx.draw(G)
        if show:
            plt.show()
...
class Explanation:
    ...
    def set_enclosing_subgraph(self, subgraph):
        ...
        if isinstance(subgraph, EnclosingSubgraph):
            ...
            self.enc_subgraph = EnclosingSubgraph(
                nodes=nodes, edge_index=data.edge_index, inv=None, edge_mask=None
            )
    ...
    def apply_subgraph_mask(
        self, mask_node: Optional[bool] = False, mask_edge: Optional[bool] = False
    ):
        """
        ...
        if mask_edge:
            ...
            self.edge_imp = self.edge_imp[
                mask_inds
            ]  # Perform masking on current member
        if mask_node:
    ...
    def visualize_node(
        ...
        assert (
            self.node_idx is not None
        ), "visualize_node only for node-level explanations, but node_idx is None"
        ...
        kadd_hop_neighborhood = gxai_utils.khop_subgraph_nx(
            G=wholeG, num_hops=num_hops + additional_hops, node_idx=self.node_idx
        )
        ...
        if heat_by_exp:
            if self.node_imp is not None:
                ...
                for i in subG.nodes:
                    if i in self.enc_subgraph.nodes:
                        if isinstance(
                            self.node_imp[self.node_reference[i]], torch.Tensor
                        ):
                            if self.node_imp[self.node_reference[i]].dim() > 0:
                                c = node_agg(
                                    self.node_imp[self.node_reference[i]]
                                ).item()
                            else:
            ...
            if self.edge_imp is not None:
                whole_edge_index, _ = subgraph(
                    kadd_hop_neighborhood, edge_index=graph_data.edge_index
                )

                # Need to match edge indices across edge_index and edges in graph
                tuple_edge_index = [
                    (whole_edge_index[0, i].item(), whole_edge_index[1, i].item())
                    for i in range(whole_edge_index.shape[1])
                ]

                _, emask = remove_duplicate_edges(self.enc_subgraph.edge_index)
                # Remove self loops:
                emask_2 = torch.logical_not(
                    self.enc_subgraph.edge_index[0, :]
                    == self.enc_subgraph.edge_index[1, :]
                )
                emask = emask & emask_2
                ...
                masked_out_by_rmdup = self.enc_subgraph.edge_index[
                    :, torch.logical_not(emask)
                ]
                ones_in_rmdup = self.edge_imp[torch.logical_not(emask)].nonzero(
                    as_tuple=True
                )[0]
                ...
                positive_edges = [
                    (
                        trimmed_enc_subg_edge_index[0, e].item(),
                        trimmed_enc_subg_edge_index[1, e].item(),
                    )
        ...
        nx.draw(
            subG.subgraph(self.node_idx), pos, node_color="red", node_size=400, ax=ax
        )

        if norm_imps:
            self.node_imp, self.edge_imp, self.feature_imp = (
                save_imps[0],
                save_imps[1],
                save_imps[2],
            )
