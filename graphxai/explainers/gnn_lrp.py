...
class GNN_LRP(_BaseDecomposition):
        ...
        **kwargs
        ...
        walk_indices_list = torch.tensor(
            self.walks_pick(
                ...
                list(range(edge_index_with_loop.shape[1])),
                num_layers=self.L,
            ),
            device=self.device,
        )
        ...
        def compute_walk_score():
                ...
                if hasattr(modules[0], "nn"):
                    for j, fc_step in enumerate(gamma_module.fc_steps):
                        ...
                        if hasattr(fc_modules[0], "weight"):
                            ...
                            fc_modules[0].weight.data = (
                                ori_fc_weight + gamma_ * ori_fc_weight
                            )

                elif isinstance(modules[0], GCNConv):
                    ori_gnn_weights.append(modules[0].lin.weight.data)
                    # (.)^ = (.) + \gamma * \rho(.)
                    gamma_module.lin.weight.data = (
                        ori_gnn_weights[i] + gamma_ * ori_gnn_weights[i].relu()
                    )
                else:  # Should be Linear layer
                    ori_gnn_weights.append(modules[0].weight.data)
                    # (.)^ = (.) + \gamma * \rho(.)
                    gamma_module.weight.data = (
                        ori_gnn_weights[i] + gamma_ * ori_gnn_weights[i].relu()
                    )
            ...
            for i, fc_step in enumerate(fc_steps):
                ...
                if hasattr(modules[0], "weight"):
                    ...
                    # (.)^ = (.) + \gamma * \rho(.)
                    gamma_module.weight.data = (
                        ori_fc_weights[i] + gamma_ * ori_fc_weights[i].relu()
                    )
                else:
                    ori_fc_weights.append(
                        None
                    )  # If layer does not have weight attribute
            ...
            for walk_indices in walk_indices_list:  # Iterate over each walk
                ...
                for i, walk_step in enumerate(walk_steps):  # Iterate over layers:
                    ...
                    if hasattr(modules[0], "nn"):  # Only for GINs:
                        ...
                        p2 = GraphSequential(
                            *gnn_gamma_modules[i].fc_steps[1]["module"]
                        )(q1)
                ...
                # --- FC LRP_gamma ---
                for i, fc_step in enumerate(
                    fc_steps
                ):  # Compute forward passes over FC for given walk
                    modules = fc_step["module"]
                    # **************************
                    std_h = (
                        nn.Sequential(*modules)(h)
                        if i != 0
                        else GraphSequential(*modules)(
                            h,
                            torch.zeros(
                                h.shape[0], dtype=torch.long, device=self.device
                            ),
                        )
                    )
                    ...
                    # --- gamma ---
                    s = (
                        fc_gamma_modules[i](h)
                        if i != 0
                        else fc_gamma_modules[i](
                            h,
                            torch.zeros(
                                h.shape[0], dtype=torch.long, device=self.device
                            ),
        ...
        exp = Explanation(edge_imp=edge_scores, node_idx=node_idx)
    ...
    def get_explanation_graph(
        ...
        **kwargs
        ...
        def compute_walk_score():
            ...
            for walk_indices in walk_indices_list:  # Iterate over each walk
                ...
                for i, walk_step in enumerate(walk_steps):  # Iterate over layers:
                    ...
                    if i == (len(walk_step) - 1):
                        # Compute h propagation differently if we're at the last GNN layer
                        std_h = GraphSequential(*modules)(
                            h,
                            torch.zeros(
                                2, h.shape[0], dtype=torch.long, device=self.device
                            ),
                        )

                        s = gnn_gamma_modules[i](
                            h,
                            torch.zeros(
                                2, h.shape[0], dtype=torch.long, device=self.device
                            ),
        ...
        walks = {
            "ids": walk_indices_list,
            "score": walk_scores_tensor_list,
        }  # One scalar score for each walk
        ...
        edge_scores = self.__parse_edges(
            walks, edge_ind_range, label, agg=edge_aggregator
        )

        exp = Explanation(edge_imp=edge_scores)
        # exp.edge_imp = edge_scores
        exp.set_whole_graph(
            Data(x=x, edge_index=edge_index_with_loop)
        )  # Make graph with self-loops
        exp._walk_ids = walks["ids"]
        exp._walk_scores = walks["score"]
    ...
    def __parse_edges(
        ...
        for i in range(walk_ids.shape[0]):  # Over all walks:
            ...
            for wn in walk:
                index_in_mask = (
                    (mask_inds == wn.item()).nonzero(as_tuple=True)[0].item()
                )
