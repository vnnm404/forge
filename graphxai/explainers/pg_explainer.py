...
class PGExplainer(_BaseExplainer):
    ...
    def __init__(
        ...
        self.elayers = nn.ModuleList(
            [nn.Sequential(nn.Linear(in_channels, 64), nn.ReLU()), nn.Linear(64, 1)]
        ).to(device)

    def __concrete_sample(
        self, log_alpha: torch.Tensor, beta: float = 1.0, training: bool = True
    ):
        """
    ...
    def __emb_to_edge_mask(
        ...
        #     print(tester)
        prob_with_mask = self._predict(
            x, edge_index, forward_kwargs=forward_kwargs, return_type="prob"
        )
        self._clear_masks()
    ...
    def train_explanation_model(self, dataset: Data, forward_kwargs: dict = {}):
        ...
        def loss_fn(prob: torch.Tensor, ori_pred: int):
            ...
            entropy = -edge_mask * torch.log(edge_mask) - (1 - edge_mask) * torch.log(
                1 - edge_mask
            )
        ...
        if self.explain_graph:  # Explain graph-level predictions of multiple graphs
            ...
            with torch.no_grad():
                ...
                for gid in tqdm.tqdm(dataset_indices):
                    ...
                    pred_label = self._predict(
                        data.x, data.edge_index, forward_kwargs=forward_kwargs
                    )
            ...
            for epoch in range(self.max_epochs):
                ...
                pred_list = []
                tmp = float(
                    self.t0 * np.power(self.t1 / self.t0, epoch / self.max_epochs)
                )
                self.elayers.train()
                optimizer.zero_grad()
            ...
            with torch.no_grad():
                ...
                pred_dict = dict(
                    zip(explain_node_index_list, label[explain_node_index_list])
                )
            ...
            for iter_idx, node_idx in tqdm.tqdm(enumerate(explain_node_index_list)):
                ...
                emb = self._get_embedding(
                    X[subset], sub_edge_index, forward_kwargs=forward_kwargs
                )
            ...
            for epoch in range(self.max_epochs):
                ...
                for iter_idx, node_idx in tqdm.tqdm(enumerate(x_dict.keys())):
                    ...
                    loss_tmp = loss_fn(
                        prob_with_mask[node_idx_dict[node_idx]], pred_dict[node_idx]
                    )
                    loss_tmp.backward()
                    # loss += loss_tmp.item()
    ...
    def get_explanation_node(
        ...
        **_,
        ...
        khop_info = _, _, _, sub_edge_mask = k_hop_subgraph(
            node_idx,
            self.num_hops,
            edge_index,
            relabel_nodes=False,
        ...
        exp = Explanation(
            node_imp=node_mask_from_edge_mask(
                khop_info[0], khop_info[1], edge_imp.bool()
            ),
            edge_imp=edge_imp,
            node_idx=node_idx,
        )
    ...
    def get_explanation_graph(
        ...
        exp = Explanation(
            node_imp=node_mask_from_edge_mask(
                torch.arange(x.shape[0], device=x.device), edge_index
            ),
            edge_imp=edge_mask,
        )
