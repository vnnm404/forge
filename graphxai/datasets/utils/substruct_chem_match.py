...
C_vec = torch.zeros(14)
C_vec[0] = 1
O_vec = torch.zeros(14)
O_vec[1] = 1
Cl_vec = torch.zeros(14)
Cl_vec[2] = 1
H_vec = torch.zeros(14)
H_vec[3] = 1
N_vec = torch.zeros(14)
N_vec[4] = 1
F_vec = torch.zeros(14)
F_vec[5] = 1
Br_vec = torch.zeros(14)
Br_vec[6] = 1
I_vec = torch.zeros(14)
I_vec[9] = 1
...
def match_substruct_mutagenicity(
    G: nx.Graph, substructure: nx.Graph = MUTAG_NO2, nh2_no2=0
):
    """
    ...
    for iso in matcher.find_isomorphisms():
        ...
        for k, v in iso.items():
            if v != 0:
                continue  # Ensures we're checking what maps to v=0
...
def match_aliphatic_halide(G: nx.Graph):
    ...
    for n, data in G.nodes(data=True):
        ...
        for c in all_cycles:
            if (other_end in c) and len(
                c
            ) > 4:  # Roughly determines if connected atom is in aromatic ring?
                break
        else:
            continue
        ...
        # print(X)
        if (
            torch.norm(X - Cl_vec).item() == 0
            or torch.norm(X - Br_vec).item() == 0
            or torch.norm(X - I_vec).item() == 0
            or torch.norm(X - F_vec).item() == 0
        ):
            matches.append(n)
...
def match_azo_type(G: nx.Graph):
    ...
    for e1, e2 in G.edges():
        ...
        if (
            (G.degree[e1] == G.degree[e2])
            and (G.degree[e2] == 2)
            and torch.norm(e1X - N_vec).item() == 0
            and torch.norm(e2X - N_vec).item() == 0
        ):
            matches.append(torch.as_tensor([e1, e2], dtype=int))
