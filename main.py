...
if __name__ == "__main__":
    ...
    if args.save_explanation_graphml:
        # save the first explanation to graphml
        for i in range(1 + 800, 11 + 800):
            save_to_graphml(
                data=dataset[i][0],
                explanation=pred_explanations[i],
                outdir=args.save_explanation_graphml,
                fname=f"{args.exp_name}_{i}_complexes.graphml",
            )
    ...
    if args.save_explanation_graphml:
        ...
        for i in range(1 + 800, 11 + 800):
            save_to_graphml(
                data=dataset[i][0],
                explanation=ground_truth_explanations[i],
                outdir=args.save_explanation_graphml,
                fname=f"{args.exp_name}_{i}_gt.graphml",
                is_gt=True,
            )
