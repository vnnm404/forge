...
def graph_exp_acc(
    gt_exp: Explanation, generated_exp: Explanation, threshold=0.8
) -> float:
    """
    Args:
...
def graph_exp_faith(
    ...
    if generated_exp.feature_imp is not None:
        ...
        top_k_features = generated_exp.feature_imp.topk(
            int(generated_exp.feature_imp.shape[0] * top_k)
        )[1]
        node_map = [
            k
            for k, v in generated_exp.node_reference.items()
            if v == generated_exp.node_idx
        ][0]

        # Getting the softmax vector for the original graph
        org_vec = model(shape_graph.get_graph().x, shape_graph.get_graph().edge_index)[
            generated_exp.node_idx
        ]
        ...
        rem_features = torch.Tensor(
            [
                i
                for i in range(shape_graph.get_graph().x.shape[1])
                if i not in top_k_features
            ]
        ).long()
        pert_x[node_map, rem_features] = torch.normal(
            0, 0.1, pert_x[node_map, rem_features].shape
        )
    ...
    if generated_exp.node_imp is not None:
        ...
        top_k_nodes = generated_exp.node_imp.topk(
            int(generated_exp.node_imp.shape[0] * top_k)
        )[1]
        ...
        for node in range(generated_exp.node_imp.shape[0]):
            if node not in top_k_nodes:
                rem_nodes.append(
                    [k for k, v in generated_exp.node_reference.items() if v == node][0]
                )
        ...
        pert_x[rem_nodes] = torch.zeros_like(
            pert_x[rem_nodes]
        )  # torch.normal(0, 0.1, pert_x[rem_nodes].shape)
        pert_vec = model(pert_x, shape_graph.get_graph().edge_index)
    ...
    GEF = (
        1
        - torch.exp(
            -F.kl_div(org_softmax.log(), pert_softmax, None, None, "sum")
        ).item()
    )
...
def calculate_delta(
    ...
    for n_id in train_set[torch.randperm(train_set.size()[0])][:100]:
        try:
            pert_edge_index = rewire_edges(
                shape_graph.get_graph().edge_index, node_idx=n_id.item(), num_nodes=1
            )
        except:
            continue
        pert_x = shape_graph.get_graph().x.clone()
        ...
        # ipdb.set_trace()
        org_vec = F.softmax(
            model(shape_graph.get_graph().x, shape_graph.get_graph().edge_index)[n_id],
            dim=-1,
        )
        org_pred = torch.argmax(org_vec)
        pert_vec = F.softmax(model(pert_x, pert_edge_index)[n_id], dim=-1)
        ...
        if org_pred.item() == pert_pred.item():
            if rep == "softmax":
                ...
                raise NotImplementedError(
                    "Intermediate model check will be implemented in the future version!"
                )

            elif rep == "final":
                raise NotImplementedError(
                    "Final embedding check will be implemented in the future version!"
                )
...
def graph_exp_stability(
    ...
    for run in range(num_run):
        ...
        try:
            pert_edge_index = rewire_edges(
                shape_graph.get_graph().edge_index,
        ...
        if check_delta(shape_graph, rep, pert_x, pert_edge_index, node_id, delta):
            ...
            cam_pert_exp.node_imp = cam_pert_exp.node_imp / torch.max(
                cam_pert_exp.node_imp
            )
            ...
            try:
                if generated_exp.node_imp.shape == cam_pert_exp.node_imp.shape:
                    ...
                    GES.append(
                        1
                        - F.cosine_similarity(
                            ori_exp_mask.reshape(1, -1), pert_exp_mask.reshape(1, -1)
                        ).item()
                    )
                else:
                    all_nodes = [
                        *intersection(
                            [*generated_exp.node_reference],
                            [*cam_pert_exp.node_reference],
                        )
                    ]
                    ...
                    for i, n_id in enumerate(all_nodes):
                        if n_id in [*generated_exp.node_reference]:
                            ori_exp_mask[i] = generated_exp.node_imp[
                                generated_exp.node_reference[n_id]
                            ].item()
                        if n_id in [*cam_pert_exp.node_reference]:
                            pert_exp_mask[i] = cam_pert_exp.node_imp[
                                cam_pert_exp.node_reference[n_id]
                            ].item()
                    topk, indices = torch.topk(ori_exp_mask, top_feat)
                    ori_exp_mask = torch.zeros_like(ori_exp_mask).scatter_(
                        0, indices, topk
                    )
                    ori_exp_mask[ori_exp_mask.topk(top_feat)[1]] = 1
                    topk, indices = torch.topk(pert_exp_mask, top_feat)
                    pert_exp_mask = torch.zeros_like(pert_exp_mask).scatter_(
                        0, indices, topk
                    )
