...
def make_edge_imp(subg_edge_index, subg_nodes, node_imp):
    ...
    for i in range(subg_edge_index.shape[1]):
        if (subg_edge_index[0, i].item() in pos_nodes) and (
            subg_edge_index[1, i].item() in pos_nodes
        ):
            edge_mask[i] = 1
...
class BAHouses(NodeDataset):
    ...
    def __init__(
        self, model_layers, n: int, m: int, num_houses: int, seed: Optional[int] = 1234
    ):
        ...
        train_mask, rem_mask = train_test_split(
            list(range(self.n)), test_size=0.3, random_state=1234
        )

        test_mask, valid_mask = train_test_split(
            rem_mask, test_size=1.0 / 3, random_state=5678
        )

        self.fixed_train_mask = torch.tensor(
            [i in train_mask for i in range(n)], dtype=torch.bool
        )
        self.fixed_valid_mask = torch.tensor(
            [i in valid_mask for i in range(n)], dtype=torch.bool
        )
        self.fixed_test_mask = torch.tensor(
            [i in test_mask for i in range(n)], dtype=torch.bool
        )
    ...
    def __generate_explanations(self):
        ...
        for i in range(self.n):
            ...
            exp.node_idx = i
            # Edge imp is any
            exp.edge_imp = make_edge_imp(
                enc_subgraph.edge_index, enc_subgraph.nodes, exp.node_imp
            )
