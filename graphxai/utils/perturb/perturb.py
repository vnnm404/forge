...
def rewire_edges(
    ...
    rewired_edge_index = to_undirected(
        torch.as_tensor(list(rewired_G.edges)).t().long()
    )
...
def PGM_perturb_node_features(
    ...
    x_pert[:, cont_dims] = torch.clamp(
        x_pert[:, cont_dims], min=torch.zeros(1, c).to(device), max=max_val
    )
