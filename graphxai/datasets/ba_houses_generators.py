...
def generate_explanations_BAHouses(data, node_idx, num_hops):
    ...
    khop_info = k_hop_subgraph(
        node_idx=int(node_idx), num_hops=num_hops, edge_index=data.edge_index
    )
    ...
    node_imp = torch.tensor(
        [1 if data.house[i] == house_code else 0 for i in subgraph_nodes],
        dtype=torch.long,
    )
    ...
    for i in range(sub_edge_idx.shape[1]):
        ...
        edge_mask[i] = ((house[e1] == house_code) and (house[e2] == house_code)).type(
            torch.long
        )

    exp = Explanation(node_imp=node_imp, edge_imp=edge_mask, node_idx=node_idx)
...
def generate_BAHouses_graph_local(
    ...
    for i in range(n):
        G, in_house, running_house_code = plant_house_local(
            G,
            node_idx=i,
            in_house=in_house,
            k=k,
    ...
    if label_strategy == 0:
        if in_hood_numbering:
            ...
            for n in range(len(G.nodes)):
                ...
                num_unique_houses = len(
                    np.unique([G.nodes[ni]["house"] for ni in nodes_in_house])
                )
...
def plant_house_local(G, node_idx, in_house, k, num_hops, running_house_code):
    ...
    unique_houses_present = np.unique([G.nodes[n]["house"] for n in khop_in_house])
    ...
    for _ in range(to_fill):
        ...
        G, in_house = plant_one_house(G, pivot, in_house, house_code=running_house_code)
...
if __name__ == "__main__":
    if sys.argv[1] == "local":
        (
            ...
            n=50, m=1, k=1, num_hops=2, get_data=False, in_hood_numbering=True
        )
        ...
        (
            ...
            n=50, m=1, num_houses=3, num_hops=2, get_data=False
        )
