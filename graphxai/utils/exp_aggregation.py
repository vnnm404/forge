...
def aggregate_explanations(
    ...
    if ref_exp.feature_imp is not None:
        feature_imp = feature_aggregator(
            torch.stack([exp.feature_imp for exp in exp_list]), dim=0
        )[0]

    if ref_exp.node_imp is not None:
        node_imp = node_aggregator(
            torch.stack([exp.node_imp for exp in exp_list]), dim=0
        )[0]

    if ref_exp.edge_imp is not None:
        edge_imp = edge_aggregator(
            torch.stack([exp.edge_imp for exp in exp_list]), dim=0
        )[0]
