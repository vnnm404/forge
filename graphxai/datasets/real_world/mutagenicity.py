...
from graphxai.datasets.utils.substruct_chem_match import (
    match_NH2,
    ...
    def __init__(
        ...
        for i in range(len(self.graphs)):
            edge_idx, _, node_mask = remove_isolated_nodes(
                self.graphs[i].edge_index, num_nodes=self.graphs[i].x.shape[0]
            )
        ...
        for i in range(len(self.graphs)):
            ...
            if test:
                if molG.number_of_nodes() != self.graphs[i].x.shape[0]:
                    ...
                    print(
                        "edge index unique:",
                        torch.unique(self.graphs[i].edge_index).tolist(),
                    )
            ...
            for m in all_matches:
                ...
                exp = Explanation(node_imp=node_imp.float(), edge_imp=edge_imp.float())
