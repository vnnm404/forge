...
def optimize_homophily(
    ...
    for i in range(disconnected_batch_size):
        ...
        while (
            if_edge_exists(edge_index, c1, c2)
            or torch.any((nc_list1 == c1) & (nc_list2 == c2))
            or (label[c1] != label[c2]).item()
        ):
            c1, c2 = random.choice(nodes).item(), random.choice(nodes).item()
    ...
    for i in range(epochs):
        ...
        c_cos_sim = F.cosine_similarity(
            to_opt[edge_index.t()[c_inds][:, 0]], to_opt[edge_index.t()[c_inds][:, 1]]
        )
        nc_cos_sim = F.cosine_similarity(to_opt[nc_list1], to_opt[nc_list2])
        diff_label_sim = F.cosine_similarity(
            to_opt[edge_index.t()[nc_inds][:, 0]], to_opt[edge_index.t()[nc_inds][:, 1]]
        )
        optimizer.zero_grad()
        loss = -homophily_coef * c_cos_sim.mean() + (homophily_coef) * (
            nc_cos_sim.mean() + diff_label_sim.mean()
        )
