...
def visualize_subgraph_explanation(
    ...
    if edge_weights is None:
        ...
        edge_index, edge_attr = remove_self_loops(
            edge_index, torch.tensor(edge_weights)
        )
    ...
    G, node_map = to_networkx_conv(
        data, to_undirected=True, remove_self_loops=True, get_map=True
    )
    ...
    if ax is None:
        if weight_map:
            nx.draw(
                G,
                pos,
                node_color=node_weights_subgraph,
                node_size=400,
                cmap=plt.cm.Blues,
            ...
            nx.draw_networkx_labels(
                G, pos, labels=get_node_weights_dict(node_weights, subgraph_nodes)
            )
        else:
            nx.draw(
                G,
        ...
        if node_idx is not None:
            nx.draw(G.subgraph(node_idx), pos, node_color="yellow", node_size=400)

        if edge_weights is not None:
            sm = plt.cm.ScalarMappable(
                cmap=plt.cm.Reds,
                norm=plt.Normalize(
                    vmin=min(edge_attr.tolist()), vmax=max(edge_attr.tolist())
                ),
            )
            plt.colorbar(sm, shrink=0.75)
        ...
        if weight_map:
            ...
            nx.draw_networkx_labels(
                G, pos, labels={n: rev_map[n] for n in G.nodes}, ax=ax
            )

        if node_idx is not None:
            nx.draw(
                G.subgraph(node_idx), pos, node_color="yellow", node_size=400, ax=ax
            )
...
def visualize_subgraph_explanation_w_edge(
    subgraph_nodes,
    ...
    edge_index, edge_attr = remove_self_loops(
        subgraph_edge_index, torch.tensor(edge_weights)
    )

    # Instantiate data object
    data = Data(
        x=subgraph_nodes.reshape(-1, 1), edge_index=edge_index, edge_attr=edge_attr
    )
...
def visualize_categorical_graph(
    data: Data = None,
    nx_graph: nx.classes.graph.Graph = None,
    node_idx: int = None,
    show: bool = True,
):
    """
    ...
    assert (data is not None) or (
        nx_graph is not None
    ), "Either data or nx_graph args must not be none."
    ...
    if node_idx is not None:
        nx.draw(bigG.subgraph(node_idx), pos, node_color="yellow", node_size=400)
...
def parse_GNNLRP_explanations(
    ret_tuple, edge_index, label_idx, edge_agg=np.sum, node_agg=lambda x: np.sum(x)
):
    ...
    edge_maps = [
        [] if e in unique_edges else [0] for e in range(edge_masks[0].shape[0])
    ]
    ...
    **kwargs,
    ...
    if self.molecule:
        if nolabel:
            ...
            node_labels = {
                n: f"{n}:{self.table(atomic_num[n].int().item())}" for n in G.nodes()
            }
            nx.draw_networkx_labels(G, pos, labels=node_labels, **kwargs)
    else:
        if not nolabel:
