...
class ShapeGGen(NodeDataset):
    ...
    def __init__(
        ...
        **kwargs,
    ):  # TODO: turn the last three arguments into kwargs
        ...
        self.num_subgraphs = (
            10 if "num_subgraphs" not in kwargs else kwargs["num_subgraphs"]
        )
        self.prob_connection = (
            1 if "prob_connection" not in kwargs else kwargs["prob_connection"]
        )
        self.subgraph_size = (
            13 if "subgraph_size" not in kwargs else kwargs["subgraph_size"]
        )
        ...
        self.max_tries_verification = (
            5
            if "max_tries_verification" not in kwargs
            else kwargs["max_tries_verification"]
        )

        # Feature args:
        self.n_informative = (
            4 if "n_informative" not in kwargs else kwargs["n_informative"]
        )
        ...
        self.n_clusters_per_class = (
            2
            if "n_clusters_per_class" not in kwargs
            else kwargs["n_clusters_per_class"]
        )
        self.homophily_coef = (
            None if "homophily_coef" not in kwargs else kwargs["homophily_coef"]
        )

        # Sensitive feature:
        self.add_sensitive_feature = (
            True
            if "add_sensitive_feature" not in kwargs
            else kwargs["add_sensitive_feature"]
        )
        self.attribute_sensitive_feature = (
            False
            if "attribute_sensitive_feature" not in kwargs
            else kwargs["attribute_sensitive_feature"]
        )
        self.sens_attribution_noise = (
            0.25
            if "sens_attribution_noise" not in kwargs
            else kwargs["sens_attribution_noise"]
        )
        ...
        if isinstance(shape, nx.Graph):
            ...
            assert (
                shape != "random"
            ), "Multiple shapes not yet supported for bounded graph"
        ...
        if self.verify and shape != "random":
            for i in range(self.max_tries_verification):
                ...
                raise RuntimeError(
                    f"Could not build a valid graph in {self.max_tries_verification} attempts. \
        ...
        self.fixed_train_mask = torch.tensor(
            [s in train_nodes for s in range_set], dtype=torch.bool
        )
        self.fixed_test_mask = torch.tensor(
            [s in test_nodes for s in range_set], dtype=torch.bool
        )
        self.fixed_valid_mask = torch.tensor(
            [s in valid_nodes for s in range_set], dtype=torch.bool
        )
    ...
    def generate_shape_graph(self):
        ...
        if self.add_sensitive_feature:
            ...
            if self.attribute_sensitive_feature:
                ...
                sensitive = torch.where(
                    prob_change, torch.logical_not(y.bool()).long(), y
                ).float()
            else:
            ...
            self.feature_imp_true = torch.cat(
                [self.feature_imp_true, torch.zeros((1,))]
            )
        ...
        edge_index = to_undirected(
            torch.tensor(list(self.G.edges), dtype=torch.long).t().contiguous()
        )
        ...
        if self.make_explanations:
            self.explanations = [
                self.explanation_generator(n) for n in sorted(self.G.nodes)
            ]
        else:
    ...
    def explanation_generator(self, node_idx):
        ...
        original_in_num_hop = set(
            [
                self.G.nodes[n]["shape"]
                for n in khop_subgraph_nx(node_idx, 1, self.G)
                if self.G.nodes[n]["shape"] != 0
            ]
        )
        ...
        node_imp_map = {
            i: (self.G.nodes[i]["shape"] in original_in_num_hop) for i in khop_nodes
        }
        ...
        node_imp = torch.tensor(
            [node_imp_map[i.item()] for i in khop_info[0]], dtype=torch.double
        )
        ...
        for i in range(khop_info[1].shape[1]):
            ...
            one_edge_in_motif = (khop_info[1][0, i] in in_motif) or (
                khop_info[1][1, i] in in_motif
            )
            node_idx_in_motif = node_idx in in_motif
            one_end_of_edge_is_nidx = (khop_info[1][0, i] == node_idx) or (
                khop_info[1][1, i] == node_idx
            )

            if (one_edge_in_motif and one_end_of_edge_is_nidx) and (
                not node_idx_in_motif
            ):
                edge_imp[i] = 1
    ...
    def visualize(self, shape_label=False, ax=None, show=False):
        ...
        pos = nx.spring_layout(
            self.G, seed=1234
        )  # Seed to always be consistent in output
        # _, ax = plt.subplots()
