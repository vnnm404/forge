...
def incr_on_unique_houses(
    nodes_to_search, G, num_hops, attr_measure, lower_bound, upper_bound
):
    # G = G.copy()
    ...
    for n in nodes_to_search:
        ...
        num_unique = (
            unique_shapes.shape[0] - 1 if 0 in unique_shapes else unique_shapes.shape[0]
        )
...
def ba_around_shape(shape: nx.Graph, add_size: int, show_subgraphs: bool = False):
    ...
    for i in range(add_size):
        ...
        )  # Just add one edge b/c shape is undirected
...
def build_bound_graph(
    ...
    **kwargs,
    ...
    show_subgraphs = (
        False
        if ("show_subgraphs" not in kwargs) or num_subgraphs > 10
        else kwargs["show_subgraphs"]
    )
    ...
    for i in range(num_subgraphs):
        ...
        s = ba_around_shape(
            current_shape, add_size=subi_size, show_subgraphs=show_subgraphs
        )
    ...
    sample_mask = np.random.binomial(
        n=2, p=prob_connection, size=len(connections)
    ).astype(bool)
    ...
    for i, j in tqdm.tqdm(iter_edges):
        ...
        deg_dist = np.array(
            [
                (subgraphs[i].degree(ni) + subgraphs[j].degree(nj))
                for ni, nj in possible_edges
            ]
        )
        ...
        while np.sum(running_mask) > 0:
            ...
            if np.sum(running_mask) > 0:
                deg_dist = (
                    deg_dist + old_deg / np.sum(running_mask) * running_mask
                ) * running_mask
            # -----------
            ...
            for t in list(original_shapes[i].nodes) + list(original_shapes[j].nodes):
                khop_union = khop_union.union(
                    set(khop_subgraph_nx(node_idx=t, num_hops=1, G=G))
                )
...
if __name__ == "__main__":
    ...
    train_mask, test_mask = train_test_split(
        torch.tensor(range(data.x.shape[0])), test_size=0.2, stratify=data.y
    )
    train_tensor, test_tensor = torch.zeros(data.y.shape[0], dtype=bool), torch.zeros(
        data.y.shape[0], dtype=bool
    )
    ...
    for epoch in range(1, 400):
        ...
        all_rec.append(rec)
        print(
            f"Epoch: {epoch:03d}, Loss: {loss:.4f}, Test F1: {f1:.4f}, Test Acc: {acc:.4f}, P: {prec:.4f}, R: {rec:.4f}"
        )
