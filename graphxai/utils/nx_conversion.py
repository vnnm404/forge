...
def whole_graph_mask_to_subgraph(
    node_mask, edge_mask=None, subgraph_nodes=None, subgraph_eidx=None
):
    ...
    subgraph_node_mask = (
        torch.tensor(
            [n.item() in nodes.tolist() for n in subgraph_nodes], dtype=torch.bool
        )
...
def khop_subgraph_nx(node_idx: int, num_hops: int, G: nx.Graph):
    """
