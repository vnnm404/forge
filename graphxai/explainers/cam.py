...
class CAM(_BaseDecomposition):
    ...
    def get_explanation_node(
        ...
        if label is None:
            if y is None:
                label = int(
                    self.__forward_pass(x, edge_index, forward_kwargs)
                    .argmax(dim=1)
                    .item()
                )
            else:
        ...
        walk_steps, _ = self.extract_step(
            x, edge_index, detach=False, split_fc=False, forward_kwargs=forward_kwargs
        )
        ...
        exp = Explanation(node_imp=cam, node_idx=node_idx)
    ...
    def get_explanation_link(self, x, node_idx):
        raise NotImplementedError(
            "Explanations for links is not implemented for Class-Activation Mapping"
        )
...
class GradCAM(_BaseDecomposition):
    ...
    def get_explanation_node(
        ...
        if label is None:
            pred = self.__forward_pass(x, y, edge_index, forward_kwargs)[0][
                node_idx, :
            ].reshape(1, -1)
        ...
        exp = Explanation(node_idx=node_idx)
        exp.set_enclosing_subgraph(khop_info)
        ...
        if average_variant:
            ...
            assert layer < len(
                walk_steps
            ), "Layer must be an index of convolutional layers"
    ...
    def get_explanation_link(self, x, node_idx):
        raise NotImplementedError(
            "Explanations for links is not implemented for Gradient Class-Activation Mapping"
        )
    ...
    def get_explanation_graph(
        ...
        pred, loss = self.__forward_pass(
            x,
            torch.tensor([label], dtype=torch.long).to(x.device),
            edge_index,
            forward_kwargs,
        )
        ...
        if node_idx is None:  # Need to compute for entire graph:
            ...
            for n in range(self.N):
                node_explanations.append(
                    self.__exp_node(n, walk_steps, layer, gradients)
                )
    ...
    def __exp_node(self, node_idx, walk_steps, layer, gradients):
        ...
        L_cam_n = F.relu(
            torch.matmul(gradients, F_l_n)
        )  # Combine gradients and activations
