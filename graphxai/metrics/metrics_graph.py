...
def graph_exp_acc_graph(
    gt_exp: List[Explanation], generated_exp: Explanation, node_thresh_factor=0.1
) -> float:
    """
...
def graph_exp_faith_graph(
    ...
    if False:  # generated_exp.feature_imp is not None:
        ...
        rem_features = torch.Tensor(
            [i for i in range(X.shape[1]) if i not in top_k_features]
        ).long()

        pert_x[generated_exp.node_idx, rem_features] = perturb_node_features(
            x=pert_x,
            node_idx=generated_exp.node_idx,
    ...
    if generated_exp.edge_imp is not None:
        ...
        # Getting the softmax vector for the perturbed graph
        pert_vec = model(
            X, edge_index.to(device), **forward_kwargs
