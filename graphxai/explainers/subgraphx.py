...
class SubgraphX(_BaseExplainer):
    ...
    def get_explanation_node(
        ...
        value_func = self._prob_score_func_node(
            node_idx=self.mcts_state_map.node_idx, target_class=label
        )
        ...
        payoff_func = self.get_reward_func(
            wrap_value_func, node_idx=self.mcts_state_map.node_idx, explain_graph=False
        )
    ...
    def get_explanation_graph(
        ...
        def wrap_value_func(data):
            return value_func(
                x=data.x, edge_index=data.edge_index, forward_kwargs=forward_kwargs
            )
    ...
    def __parse_results(self, best_subgraph, edge_index):
        ...
        subgraph_nodes = (
            torch.tensor([map[c] for c in best_subgraph.coalition], dtype=torch.long)
            if map is not None
