...
def motif_id_label(G, num_hops):
    ...
    def get_label(node_idx):
        ...
        motif_in_khop = torch.sum(
            torch.unique([G.nodes[ni]["motif_id"] for ni in nodes_in_khop])
        ).item()
...
def number_motif_equal_label(G, num_hops, equal_number=1):
    def get_label(node_idx):
        nodes_in_khop = khop_subgraph_nx(node_idx, num_hops, G)
        num_unique_houses = torch.unique(
            [G.nodes[ni]["shape"] for ni in nodes_in_khop if G.nodes[ni]["shape"] > 0]
        ).shape[0]
...
def logical_edge_feature_label(G, num_hops=None, feature_method="median"):
    ...
    def get_label(node_idx):
        ...
        return torch.tensor(
            int(num_unique_houses == 1 and x1[node_idx] > split), dtype=torch.long
        )
